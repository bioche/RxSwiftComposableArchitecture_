      ...
      let store = Store(
        ...
        reducer: Reducer<State, TestAction, Void> { state, action, _ in
          ...
          return
            effects
            .do(
              onCompletion: { _ in longLivingEffects[key]?.remove(id) },
              onSubscribe: { longLivingEffects[key, default: []].insert(id) },
              onDispose: { longLivingEffects[key]?.remove(id) }
            )
      ...
      if !receivedActions.isEmpty {
        ...
        _XCTFail(
          """
            ...
            make sure that all of them are completed by the end
