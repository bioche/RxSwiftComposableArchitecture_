...
extension TestStore where LocalState: Equatable {
  ...
  public func assert(
    ...
    // CompositeDisposable seems to be a keyed collection of disposables where remove by key methods exist
    ...
    func runReducer(action: Action) {
      ...
      var disposableReferenceKey: CompositeDisposable.DisposeKey?
      
      disposable = effect.subscribe(onNext: {
        receivedActions.append($0)
      }, onCompletion: { (_) in
        isComplete = true
        guard let disposableReferenceKey = disposableReferenceKey else { return }
        disposables.remove(for: disposableReferenceKey)
      })

      if !isComplete, let disposable = disposable {
        // we insert the disposable in the disposables collection, doing so return the reference key that we could use later on to remove the disposable from the collection
        disposableReferenceKey = disposables.insert(disposable)
      }
    }
